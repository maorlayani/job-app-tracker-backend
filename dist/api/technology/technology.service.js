...
exports.getById = exports.query = void 0;
...
//     getById
// }
async function query(techSearch) {
    try {
        const criteria = _buildCriteria(techSearch);
        const collection = await (0, db_service_1.getCollection)('technologies');
        let technologies = await collection.find({}).toArray();
        if (!technologies || !technologies.length)
            technologies = await collection.insertMany(gDefaultTechnologies);
...
exports.query = query;
function _buildCriteria(techSearch) {
    let criteria = {};
    if (techSearch) {
...
async function getById(technologyId) {
    try {
        const collection = await (0, db_service_1.getCollection)('technologies');
        const technology = await collection.findOne({ _id: new ObjectId(technologyId) });
        return technology;
    }
    catch (err) {
